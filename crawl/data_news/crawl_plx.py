...
input_dir = "../../dataset/link/plx/"
output_dir = "../../dataset/data/plx/"
os.makedirs(output_dir, exist_ok=True)


def crawl_article(url):
    """Crawl chi tiết bài viết Petrolimex"""
    try:
    ...
    # --- Tiêu đề ---
    title_tag = soup.find("h2", class_="blogDetail__title")
    title = title_tag.get_text(strip=True) if title_tag else ""

    # --- Ngày đăng ---
    date_span = soup.select_one("div.blogDetail__meta span.f-date")
    ngay_dang = date_span.get_text(strip=True) if date_span else ""

    # --- Nội dung ---
    content_parts = []
    content_div = soup.find("div", class_="entry-detail")
    if content_div:
