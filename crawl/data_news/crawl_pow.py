...
input_dir = "../../dataset/link/pow/"
output_dir = "../../dataset/data/pow/"
os.makedirs(output_dir, exist_ok=True)


def crawl_article(url):
    """Crawl chi tiết bài viết Petrotimes"""
    try:
    ...
    # --- Tiêu đề ---
    title_tag = soup.find("h1", class_="title post-title")
    title = title_tag.get_text(strip=True) if title_tag else ""

    # --- Ngày đăng ---
    date_div = soup.find("div", class_="tie-date")
    ngay_dang = date_div.get_text(strip=True) if date_div else ""

    # --- Nội dung ---
    content_parts = []

    # 1. Lấy sapo mở đầu
    sapo_div = soup.select_one("div.article-content div.sapo.clearfix")
    if sapo_div:
        sapo_text = sapo_div.get_text(" ", strip=True)
        if sapo_text:
            content_parts.append(sapo_text)

    # 2. Lấy toàn bộ nội dung trong colLeftDtails
    col_div = soup.find("div", class_="colLeftDtails")
    if col_div:
