...
input_dir = "../../dataset/link/sgt/"
# Thư mục lưu kết quả crawl chi tiết
output_dir = "../../dataset/data/sgt/"
...
def crawl_article(url):
    ...
    # --- Tiêu đề ---
    title_tag = soup.select_one("h2.itemTitle")
    title = title_tag.get_text(strip=True) if title_tag else ""

    # --- Ngày đăng ---
    date_tag = soup.select_one("span.itemAuthor")
    ngay_dang = date_tag.get_text(strip=True) if date_tag else ""

    # --- Nội dung ---
    content_parts = []
    body = soup.select_one("div.itemBody")
    if body:
        for p in body.find_all("p"):
            style = p.get("style", "").replace(" ", "").lower()
